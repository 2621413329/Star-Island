...
class Product(Base):
    ...
    purchases = relationship("IapPurchase", back_populates="product")
...
class IapPurchase(Base):
    ...
    product_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("iap_products.product_id", ondelete="RESTRICT"), index=True, nullable=False
    )
    transaction_id: Mapped[str] = mapped_column(String(255), unique=True, nullable=False)
    original_transaction_id: Mapped[str] = mapped_column(String(255), index=True, nullable=False)
    purchase_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    expires_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class UserEntitlement(Base):
    ...
    entitlement: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    status: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
