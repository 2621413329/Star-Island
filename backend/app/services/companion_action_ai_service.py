...
- story_summary_lines: 3句中文对话式陪伴语，每句≤32字，像朋友聊天对用户说话（详情页点击小人时展示）。若提供了 user_nickname，至少2句要自然融入昵称，例如「小明，今天辛苦啦」「今天生活对我们小明怎么样呀」「小明，这件事我记得哦」，避免只在句首机械加「昵称，」
...
class CompanionActionAIService:
    ...
    async def enrich(
        ...
        *,
        ...
        fallback = self._fallback(emotion_tag, event_tags, note, nickname=nickname)
        ...
        if llm:
            payload = {
                ...
                "note": note or "",
                "user_nickname": (nickname or "").strip(),
            }
            try:
                ...
                if parsed:
                    spec = self._normalize_spec(
                        parsed, emotion_tag, event_tags, note, nickname=nickname
                    )
    ...
    def _normalize_spec(
        ...
        *,
        ...
        fb = self._fallback(emotion_tag, event_tags, note, nickname=nickname)
    ...
    def _fallback(
        ...
        *,
        ...
        summary_lines = self._story_summary_lines_from_context(
            event_tags, emotion_tag, note, title, nickname=nickname
        )
        return {
            "expression": expr,
            "prop": prop,
    ...
    def _normalize_story_summary_lines(self, value: Any, fallback: list[str]) -> list[str]:
        if not isinstance(value, list):
            return fallback
        lines = [str(item).strip()[:32] for item in value if str(item).strip()]
        if len(lines) >= 3:
            return lines[:3]
        merged = lines + [line for line in fallback if line not in lines]
        ...
        *,
        ...
        if name:
            if note and len(note.strip()) >= 4:
                snippet = note.strip()[:14]
                return [
                    f"{name}，今天辛苦啦，{snippet}我都记得",
                    f"今天{tag}对我们{name}怎么样呀？",
                    f"{name}，那一刻{mood_label}，我替你收好了",
                ]
            return [
                f"{name}，今天辛苦啦",
                f"今天{detail or tag}对我们{name}怎么样呀？",
                f"{name}，{title}这件事值得记住",
            ]

        if note and len(note.strip()) >= 4:
            snippet = note.strip()[:18]
            return [
                f"今天你说的{snippet}，我都记得",
                f"那一刻你感到{mood_label}，我替你收好了",
                f"关于{detail or tag}的这件事，值得被温柔记住",
            ]
        pools = {
            "学习": [
                f"今天的{detail or '学习'}让你感到{mood_label}吧？",
                "那些努力的时刻，小岛都看见了",
                f"{title}，我会一直替你记着",
            ],
            "朋友": [
                f"和朋友有关的这一刻，你当时{mood_label}吧？",
                "友情里的小波澜，也值得被好好安放",
                f"{title}，我陪你慢慢回味",
            ],
            "运动": [
                f"运动后的你，心里是{mood_label}的吧？",
                "汗水和风声，我都帮你收进小岛了",
                f"{title}，真是闪亮的一刻",
            ],
            "家庭": [
                f"家里的这件事，让你感到{mood_label}吧？",
                "家的温度，有时藏在细节里",
                f"{title}，我替你轻轻放好",
            ],
            "兴趣": [
                f"做喜欢的事时，你显得{mood_label}吧？",
                "热爱会让小岛多一盏小灯",
                f"{title}，是很珍贵的瞬间",
            ],
            "生活": [
                "今天生活待你还温柔吗？",
                f"这一刻{mood_label}，小岛替你收好了",
                f"关于{detail or tag}的事，值得慢慢回味",
            ],
        }
        return pools.get(
            tag,
            [
                f"这一刻你感到{mood_label}吧？",
