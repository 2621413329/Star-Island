...
async def create_observation(payload: ObservationCreate, db: DBSession, current_user: User = Depends(get_current_admin)):
...
async def update_observation(record_id: uuid.UUID, payload: ObservationUpdate, db: DBSession, _: User = Depends(get_current_admin)):
...
async def delete_observation(record_id: uuid.UUID, db: DBSession, _: User = Depends(get_current_admin)):
...
async def get_observation(record_id: uuid.UUID, db: DBSession, _: User = Depends(get_current_admin)):
