...
async def text_chat(payload: AIChatRequest, _: User = Depends(get_current_admin)):
...
async def chat_with_qwen(payload: AIChatRequest, _: User = Depends(get_current_admin)):
    return ResponseModel(data=await AIService().chat(payload))


@router.post("/text-to-image", response_model=ResponseModel[AITaskResponse], summary="文生图")
async def text_to_image(payload: TextToImageRequest, _: User = Depends(get_current_admin)):
...
async def image_to_video(payload: ImageToVideoRequest, _: User = Depends(get_current_admin)):
...
async def get_ai_task(task_id: str, _: User = Depends(get_current_admin)):
